...
namespace rchia
{
    internal class ClientFactory2
    {
        ...
        public async Task<IRpcClient> CreateRpcClient(StatusContext ctx, EndpointInfo endpoint, int timeoutMilliseconds)
        {
            using var status = new StatusMessage(ctx, $"Connecting to endpoint {endpoint.Uri}...");

            return await CreateRpcClient(endpoint, timeoutMilliseconds);
        }

        public async Task<IRpcClient> CreateRpcClient(EndpointInfo endpoint, int timeoutMilliseconds)
        {
