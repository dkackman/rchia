        ...
        return await DoWorkAsync("Retrieving farm info...", async output =>
        {
            ...
            if (wallet_running)
            {
                try
                {
                    ...
                    row.Add("total_chia_farmed", FarmedAmount.AsChia("F1"));
                    row.Add("user_transaction_fees", FeeAmount.AsChia("F1"));
                    row.Add("block_rewards", (FarmerRewardAmount + PoolRewardAmount).AsChia("F1"));
                    row.Add("last_height_farmed", LastHeightFarmed.ToString("N0"));
                }
                catch
                {
                    wallet_ready = false;
            ...
            row.Add("total_plot_count", totalPlotCount.ToString());
            row.Add("total_plot_size", totalplotSize.ToBytesString("N1"));

            if (blockchain_state is not null)
            {
                row.Add("estimated_network_space", blockchain_state.Space.ToBytesString());
            }
            else
            {
                row.Add("estimated_network_space", "Unknown");
            }
            ...
            if (blockchain_state is not null && blockchain_state.Space != BigInteger.Zero)
            {
                if (totalPlotCount == 0)
                {
                    row.Add("expected_time_to_win", "Never (no plots)");
                }
                else
                {
                    ...
                    row.Add("expected_time_to_win", span.FormatTimeSpan());
